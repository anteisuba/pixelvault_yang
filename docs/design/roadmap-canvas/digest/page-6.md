...
_PixelVault · 6 在设计 · 卡片（35 卡片总线）· ② 思维导图 · 2026-09-25_

回答两个问题：**卡片不是照酒馆设计的**——卡的主体（参考图、用途、音色、变体）是我们自己为「生成一致的画面 / 视频 / 配音」做的；酒馆只在 09-19 你点名参考后，借了文字那一侧的三件。**情绪**来自 09-19 你提的「AI 按角色的语气 / 情感回复」，加上配音间本来就有的 9 档；它只服务台词稿和配音，而语音 09-17 已定后置，所以我建议把它从 35 拿掉。绿 = 已定 / 已落，紫 = 我的建议，黄虚线 = 等你定，灰虚线 = 依赖别的条目。

`已定` owner 拍过 · `已落` 代码已有 · `建议` 我的建议，等你点头 · `待定` 等你定 · `依赖` 等别的条目

**卡片：同一个角色在哪都长一个样、说一个声音**

**1 卡片是干嘛的**

- `已定` 可复用的命名实体：同一个角色在图片 / 视频 / 画布 / 配音里保持一致（v2，09-17）
- `已定` 和酒馆不是一类东西：酒馆的卡给「陪聊」用，是一张可交换的 PNG；我们的卡给「生成」用，活在自己的库里，有 id、变体、生成记录

**2 每块从哪来**

- **我们自己的（主体，v2 已落）**
  - `已落` 参考图 + 11 类用途（身份 / 姿势 / 画风 / 构图 / 背景 / 脸部特写 / 服装 / 道具 / 首帧 / 尾帧 / 自定义）
  - `已落` 音色绑定（voiceCard）· 变体树 · 版本号 · 来源记录
- **你 09-19 提的两条需求（驱动了文字那一侧）**
  - `已定` 卡片给 LLM：AI 按角色的语气 / 情感回复
  - `已定` 卡片给助手：写剧本时看得到角色详情和角色之间的关系
  - `已定` 参考酒馆补齐这一侧（你点名 sillytaverncn / launcher）
- **从酒馆借的（只有文字侧三件）**
  - `已定` 示例对白 examples：酒馆全套字段里唯一真正传达「说话口吻」的一项
  - `已定` 设定条目 lore 最小版：提到关键词才注入，不提就不占提示词
  - `已定` 多角色按参考槽分开，不把文字拼在一起（酒馆「串味」的教训）
- **明确不借**
  - `已定` PNG 自包含与导入导出 · 28 类情绪事后分类 · 设定条目的正则 / 递归 / 概率 · 酒馆的用户人设 Persona

**3 情绪为什么会出现**

- `已定` 起点：你 09-19 的「按角色的语气 / 情感回复」；配音间本来就有 9 档（平静 · 兴奋 · 耳语 · 旁白 · 对话 · 愤怒 · 悲伤 · 惊讶 · 无）
- `已定` 契约写了「14 值表演情绪」，用在三处：台词稿的情绪标 · 配音情绪 · 表情参考图的槽名
- `建议` 这三处都属于写台词 / 配音，而语音 09-17 已定整体后置 → 从 35 拿掉情绪，等做台词 / 配音那一片再定，到时直接用配音现有 9 档起步

**4 35 卡片总线本来要做什么** — 只做数据、编译和画布 op；界面归 D6

- `已定` 只认角色卡一张表；画布侧栏、@ 名单改读它
- `已定` handle：@名字 的稳定锚点；角色和背景共用一个 @ 命名空间
- `已定` 一条参考槽列表合并现在的四份图列表，每张图带用途
- `已定` summary 给人看、不进提示词；description 只写外观，开始进编译
- `已定` 编译只有一个入口，图片 / 视频 / 画布各自压平；画布加 attach_card / detach_card
- `已定` 迁移三次部署：先加列双写 → 切读 → 删旧列（删之前建 Neon 快照）
- `已定` 文字侧：示例对白按场景分块 · 设定条目 · 关系结构化存、对方出场才注入
- `依赖` 装填按钮、关系编辑、简介 / 设定条目编辑的界面 → D6 设计门

**5 等你定**

- `建议` 情绪从 35 拿掉，推到台词 / 配音那一片
- `待定` 文字侧三件（示例对白 · 设定条目 · 关系）是 35 就做，还是也推后：35 只做「画面一致」那一半（handle · 参考槽 · 编译 · attach_card），施工从 13 片缩到约 9 片
