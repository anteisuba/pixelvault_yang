/**
 * 归属追踪 —— **「这一次生成是助手备的那一枪吗」**（P3-C）。
 *
 * ## ⚠ 它管的范围在 2026-09-06 收窄了（拍板 4 推翻，切片 3a）
 * 旧口径：归属票是**看图的唯一凭证** —— 没票就不许看。现在不是了：用户 `@` 指定
 * 的任意一张一律可以直接作 `critique_result` 的目标（走请求里的 `mentionedAssets`，
 * 服务端 `resolveCritiqueTarget` 那一段）。
 * **这张票剩下的职责只有一条**：判定「这一枪是助手自己 primed 的」，从而允许它
 * **主动开口**评一张图（自动开场白那条路径）。⛔ 它不再是任何一条准入闸 ——
 * 想改看图的准入范围，去改 `mentionedAssets` 那张名单，不是改这里。
 *
 * ## 为什么仍然需要它
 * 「用户自己发的**不打扰**」这半句一个字没变：没有这张票，助手会对着用户自己
 * 点的每一枪都自说自话地评一段。而工作台里「点生成键」只有一条路径，两种情况
 * 走的是同一个函数 —— 判据只能长在这里。
 *
 * ## 判据：领一张票，只认票上写的那几条
