...
不顺手重构无关代码（发现别的问题 → 报告或立 task，不混进本次 diff）· 不改产品行为来绕过 bug（行为变更 = 产品决定，问 owner）· 高风险模块的修复照样**一次改到位**：grep 出全部调用方在同一个改动里一起改完，不为了「少动几个文件」留旧签名垫片或 fallback（CLAUDE.md Engineering Principle 1）。
