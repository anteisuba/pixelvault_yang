...
1. **Before modifying any type**: run `rg -l "from '@/types" src` (add `| wc -l` for the count) to confirm impact scope
2. **Change every consumer in the same change** when you add a required field, rename, or remove a type — the grep above is the list of files to edit, not a reason to keep the old shape alive (AGENTS.md engineering principles: no compat layers, no shims)
...
- `index.ts` — 核心共享 schema 与类型（影响面最大）；其余按域拆在同目录 `*.ts`（`ls src/types`）
