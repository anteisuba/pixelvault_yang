...
/**
 * 看图闭环的**自动那一半**（P3-C）：盯着生成结果，等**它自己备的那一枪**回来，
 * 把结果投回操作员线程，助手因此主动开口。
 *
 * ── ⚠ 它不是看图的全部入口了（拍板 4 推翻，2026-09-06 · 切片 3a）─────
 * `@` 指定的任意一张图**不经过这里**：那条路是用户在输入框里挂 chip → 请求带
 * `mentionedAssets` → 服务端 `critique_result` 按 `targetIds` 挑一张（§7）。
 * 这颗 hook 只管「助手自己 primed 的那一次回来了」这条自动路径。
 * ⛔ 所以别在这里加「用户 @ 了什么」的逻辑：那会让同一件事有两个真相源，而
 * 其中一个（这里）看不见输入框。
 *
 * ── ⛔ 没有新轮询器 ────────────────────────────────────────────────
