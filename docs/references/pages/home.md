...
| 能力段   | **钉住舞台**，随下滑换片三个能力，带步进条                     | 文字左 / 演示右**固定不交替**（同一舞台上换边会闪）；三个演示框形态各异        |
| 模型阵容 | 按图片 / 视频 / 音频 / 3D **四类分轨**                         | 此段单独用页面边距，不吃 1360 居中；只有能真正滚动的组才配箭头                 |
...
| 进场       | 上浮 70px + 淡入；产品框 110px + 从 .93 放大；模型卡错开 .07s                                    |
| scrub      | hero 图条视差滞后 · 模型轨滚过时自己横向漂 45%                                                   |
| 钉住       | 能力段三片交叉换片，步进条同步                                                                   |
