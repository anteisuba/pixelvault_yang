...
- 明暗基调：与系统统一的白色浅色工作台。页面、输入、结果面板与浮层使用白色语义槽，以轻灰凹面、细边框和黑灰文字建立层级；避免发光渐变、玻璃效果、card/pill 堆叠和装饰性 chrome。⚠ 2026-09-03 起地台改灰底 `.workbench-ground`，装配栏/提示词区/结果监视三块面板各自浮起为白卡 `.workbench-card`（与配音间同款材质），不再是整页统一白底。
...
LoRA Generate 由 `.domain-lora` 负责工作区表面、发丝分隔、输入表面、结果监视面、弱文本、焦点和主要动作层级，并映射到系统浅色语义档；不得把页面专属结构写回全局组件皮肤，也不能因颜色统一而套用其它业务域的 card/pill 结构。
...
- 不得像 Studio Image 的底部 composer 或“结果漂浮在输入之外”的结构；LoRA Generate 的装配证据与大结果必须和输入形成稳定同屏关系。
