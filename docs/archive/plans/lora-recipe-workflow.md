...
- 一键补挂缺失 LoRA（owner 2026-06-11 追加）：extras 携带定位信息
  （resources hash / civitaiResources modelVersionId；仅 prompt 标签名
  的标"无法自动定位"）→ GET /api/lora-assets/civitai/resolve（by-hash
  端点实测核验：返回完整 version 负载、hash 大小写不敏感）→ 警告区
  逐项「挂载」按钮；应用来源图配方时也会先自动补挂可定位的 extra LoRA。
  原图权重作 scale（越界回默认），栈满（3）时未补挂项明示还原受限。
...
面板宿主就是 LoraPromptControlButton**；死组件已在本轮删除，后续不要再
恢复 `StudioLoraChip.tsx`。
