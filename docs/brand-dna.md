...
PixelVault 的一致性来自“这是同一个产品、拥有同一套可靠行为、同一套字体与颜色语义”；域之间的差异来自结构、比例、信息层级、交互关系和材质密度，**不来自各自的字体、调色板或强调色**——强调色全站只有 `--primary`（2026-09-06 owner 定）。
...
| Domain           | 一个业务域的材质、几何、密度、布局、动效性格                             | 不定义字体、不改 semantic、**不定义强调色**（全站只用 `--primary`，2026-09-06 owner 定）、不外溢 |
| Page / Component | 页面结构、标志性组件、状态细节和局部覆盖                                 | 不反向绑架其他页面                                                                               |

域和页面 token 引用全局 primitive/semantic；可覆盖的只有材质、几何、密度、布局与动效性格，**字体槽、semantic 颜色与强调色不在可覆盖范围**（强调色全站只用 `--primary`，2026-09-06 owner 定；`--modality-*` 仅 prompts 域使用，见 `references/ui-defaults.md` §2.3）。域 token 写在域根，不写 `:root`。第二个业务域需要同一视觉模式时，先分别验证，再讨论是否提取。
...
- 材质、圆角、阴影和密度（⛔ 强调色不在此列：全站只用 `--primary`）。
- 业务域布局、信息层级和动效性格。
...
同一业务域内部保持连贯；不同业务域不需要像同一个模板的换色版本。差异来自工作流、结构、比例、信息层级、交互关系与材质密度；**不来自换字体、换调色板或换强调色**。arena 已于 2026-09-17 整删，不再是业务域。
...
- 2026-09-03 · owner 拍板：字体三槽与 semantic 颜色收回品牌脊柱，域只留一个强调色；应用默认浅色；移动端等级进底线；arena 待删（2026-09-17 已整删）。默认值移交 `references/ui-defaults.md`。
