...
| S5f-B1/B3 ✅ 2026-07-11 | 磁吸（合法目标高亮 + 最近吸附，引擎命令式）+ 张口预览（卡匣卡路径：onBiteChange → 迷你清单「图集×N·音色·@名字·参考位 n/m」，超限红）。⚠ **B2 快投模式 / B4 把手热区 / 张口预览的画布节点拖拽路径未做**——需 workbench `handleNodeDrag` 跨两套拖拽系统协调，留 S5f-B 续片（真机拖拽验证需稳定 dev env）                          | 中高（两套拖拽系统：卡匣卡 pointer 引擎 / 画布节点 ReactFlow onNodeDrag）     |
