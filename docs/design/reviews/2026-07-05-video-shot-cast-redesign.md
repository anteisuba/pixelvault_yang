...
**V2-2 已交付**（2026-07-05 晚，commit `aba8cd37`）：token 内嵌 16px 缩略图落地——`MentionInput.buildChip` 从纯文本 span 升级为「缩略图 + 名字」结构：`buildThumb` 生成 `size-4` 缩略图（圆=character/voice、方=background/shot/video，对应 §9 A 权威图形语言），有图时塞 `<img>`（voice 走 `coverImage`、其余走 `mediaUrl`，与 `ReferenceTokenChip` 同源），无图时退化成端口色 70% 底纯色块；video 额外叠 ▶ SVG 角标区分动图。chip 外壳从 `rounded px-1` 换成 `rounded-full`（胶囊，对齐权威图），文字包进独立 `.mention-chip-label` span，`textContent` 契约（`@name`）不受影响——`serializeEditor` 零改动。`MentionToken` 新增可选 `thumbnailUrl` 字段；`VideoComposer.mentionTokens` 组装时补上该字段，同时把 `refToken.kind === 'keyframe'` 过滤进类型谓词——顺带修了一个**自 keyframe 提交（222772b5）起就潜伏的 tsc 类型错误**：`mentionTokens` 把 `ReferenceTokenKind`（含 `keyframe`）直接赋给 `MentionToken['kind']`（不含 `keyframe`），本次全量 tsc 首次抓到并顺手修复。测试新增 2 例（缩略图 img src 断言 + video ▶ 角标断言）。验证：定向 vitest 34 passed，全量 tsc exit 0（第一轮暴露上述潜伏错误，第二轮 0 错误干净收尾）。

**V2-3 已交付**（2026-07-06）：自动编号 + 视频可内联引用落地——`use-video-composer.ts` 的 `referenceTokens` 组装新增 `autoName(kind, slotIndex)`：character/background/shot 未命名但已有 `mediaUrl`（能拿到真实 `imageSlotIndex`）时，用 `${autoName前缀}${slotIndex+1}`（如「场景2」）生成可插入 token，编号直接复用 `imageSlotIndex`/`videoSlotIndex`（与 图N/视N 角标同一真源，不另起一套计数——这也是为什么同类 kind 的编号可能不连续，文档已明确这是对的）；用户命名优先，无媒体的未命名节点仍保留原「需命名」态（没有 payload 槽位可编号）。video 从固定 `insertable:false` 改为按 `videoSlotIndex` 自动编号「视频N」且 `insertable:true`，动作卡参考视频从此可插入组句（如「运镜完全参考 @视频1」）；keyframe 保持 projection-only 不变。i18n 新增 `StudioNode.videoComposer.autoName.{character,background,shot,video}`（zh/en/ja 三语同步）。**顺带修复一个由本次改动暴露的既有逻辑 bug**：`ReferenceTokenChip` 的 `noInsertHint`（"参考视频随连线自动发送，无需插入提示词"）此前与 `insertable` 状态无关地无条件计算，video 变为 insertable 后会在 hover 浮层里显示自相矛盾的"无需插入"提示——改为 `insertable` 为 true 时不显示该 hint。**确认既有机制无需改动**：自动编号的名字被后续正式改名后，V2-1 的漂移自动回写会按同一路径把 `@角色2` 改写成新名字，不需要专门处理"编号名"这个特例。**确认范围边界**：cast-redesign v4 §7.2⑦ 设计的"发送时自动图例"（`buildShotReferenceLegend` 模式）目前只服务于镜头(shot)图像生成节点，尚未接入 video/Seedance 生成路径（v4 §14.3 已列为未交付项）——这是另一项待办，V2-3 不在此范围内，未来接入时需确认自动编号的名字也能正确进入图例映射。测试：`use-video-composer.test.ts` 新增/更新 3 例（video 自动编号、character/background 各自 slot 编号、无媒体保留未命名）、`ReferenceTokenChip.test.tsx` 新增 1 例（insertable video 无矛盾 hint）；定向 vitest 51 passed，全量 tsc exit 0。

**V2-4 特写(closeup) 已交付**（2026-07-06，内部拆四子片，全部提交）：新 image role `closeup`=角色面部细节子参考，端到端打通。

- **V2-4a/b**（commit `facab275`）：`closeup` role（presentation 复用 character 族=`NODE_IMAGE_ROLE_TO_LEGACY_TYPE[closeup]=characterImage`、fields=[prompt]、名字字段 characterName），不进 ImageRolePicker 三择（从角色身份组＋特写生）。连线 `closeup→character`（同 voice→character 1 跳），且 closeup **只能连 character**（连 shot/seedance 一律拒绝，否则成收割静默丢的死边）；characterImage 输入受理从静态矩阵移进 `canConnectNodeTypes` 角色分支（voice OR closeup，单一真源）。spawnReference 给 closeup 盖 characterName。
- **V2-4c**（commit `4299eaa0`）：**1 跳收割** `harvestUpstreamCloseupUrls(focalNodeId,edges,nodes)`（遍历 video 直接上游 character，取其上游 closeup 的 mediaUrl；`isCloseupNode`=image+role=closeup），composer payloadImageUrls 与生成路径 StudioNodeWorkbench **同步 append**（keyframe→主 refs→closeup），两处口径一致故 图N/特写N 角标与实送对齐。composer 1 跳解析角色上游 closeup → 顶层 `closeup` token（kind 加进 ReferenceTokenChip/MentionInput/TOKEN_PORT_COLOR_VAR 各 map，方形·image 族色），auto 编号「特写N」insertable、带 parentCharacterId、edgeId=closeup→character 边（× 断特写不断视频）。＋特写 spawn（nodeType=image/role=closeup/targetNodeId=角色，同 ＋配音 范式）。
- **V2-4d**（commit `bec00cf`，owner 拍板照 mockup）：身份单元卡——角色卡改「一角色一边框卡」`IdentityUnit`（face + 其 closeups + 音色徽标 grouped + 尾部虚线＋特写 tile + caption「名字 脸·特写?·音色?」按存在构面拼）；＋特写 从角色槽角标移到单元尾部 tile。纯视觉，收割/发送不变。i18n `references.facetFace/facetCloseup/facetVoice`。

**发送机制仍不变**：胶囊→@名字纯文本，closeup 随其 character 进 image_urls。测试：graph 1 跳收割 2 例、composer closeup 表面化/auto 名 2 例、strip 分组+＋特写+身份单元 caption 4 例、连线规则 2 例；各子片全量 tsc 我方文件型净。

- **视频参考图例**（commit `da265022`，接完 §7.2⑦/§9 D 的「@特写N 图例映射」）：video 生成路径现在像 shot 一样在 finalPrompt 前追加自动图例，把实送的 图N/视N/音N 每槽绑到主体（`图2：角色「剑修」` / `图3：特写「特写3」` / `视1：视频「视频1」` / `音1：角色「剑修」的音色`），让 composer 插的 `@名字`/`@特写N`/`@视频N` 真能被模型解析。两个纯函数：`harvestUpstreamVideoImageReferences`（url→{kind,name}，收 char/bg/shot + 1 跳 closeup，keyframe 无名跳过）、`buildVideoReferenceLegend`（按 referenceImages 最终 index 编号，**未命名槽回退用同一 i18n autoName 前缀+index**，故 `@特写3`==图例`特写3` 逐字一致）。kindLabel/结构词走中文常量 `NODE_STUDIO_VIDEO_REFERENCE_LEGEND`，auto 名前缀由 StudioNodeWorkbench 用 `t('videoComposer.autoName.*')`（与 composer 同 key）传入。图例纯函数不绑常量、labels 全参传、更好测。测试 graph 51 绿、全量 tsc exit 0。⚠ 仍未做：抽共享 payload 装配（v4 §14.3），video 侧图例只在客户端生成路径拼、与 `buildShotReferenceLegend` 各自。
