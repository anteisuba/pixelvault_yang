...
| 根目录          | 画布外壳：`CanvasTopBar` / `CanvasBottomDock` / `CanvasMiniMap` / `CanvasLeftPanel` / `CanvasSurface` / `CanvasWorkspaceLayout` · `CastDock` / `CastCard` / `CanvasRosterRail` · `ScriptDocWorkspace` · `StudioNodeAssistantDock` + `CanvasOpProposalCard` · `CanvasAddMenu` · `IngestDragLayerV4`                                            |
