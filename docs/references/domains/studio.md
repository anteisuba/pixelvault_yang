...
- `/studio/node`（画布 v4，基准见 `../pages/node-canvas-v2.md`）· `/studio/lora` · `/studio/3d`（搁置）· `/studio/edit(/**)` · `/studio/enhance` / `analyze`（工具路由）。
...
- 助手宿主 = 右侧 dock，手机上是同一颗 Dock 里的全屏 Sheet（**现行施工基准见 `../pages/assistant-shell.md`**——2026-09-06 owner 定方向 C「工作日志」：覆盖式面板 inset 24 / 默认 560 · 收起态 48px 图标轨 · 顶部进度带（**助手设置入口 = 带上那颗常驻齿轮**，2026-09-07 起 ⋯ 菜单不再有这一项）· 三档确认 · 计划卡 · @ 看图（**视频档扩成三帧评审卡**）· 助手设置 persona · 检索链 `research` / `read_url` / 官方优先搜图；**第二期视频域主体已落地**（`6e91e0d0` + `48d6fecb`）；画布走自己的一套 op 与提案卡，见 `../pages/node-canvas-v2.md` §8）。
