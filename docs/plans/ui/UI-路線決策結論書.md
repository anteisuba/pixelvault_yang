...
> 查當前代碼現狀時，請先讀 `docs/plans/ui/02-現狀映射.md`，再回來看本決策書。
...
**產出物**：一份 `docs/reference/ui-components.md` 明確每個 UI 組件的使用邊界、props 規範、禁用場景。不上 Storybook（ROI 低）。
...
- 本文件**應**在以下時機被更新：
  ...
  - 某個新的產品方向出現需補充新決策
- 本文件**不應**被擴寫成設計系統完整規範 — 那是另一份文檔（可參考 `docs/reference/design-system.md`）
...
docs/plans/feature/03-工作包細分.md           ← 功能實作 WP（對 UI 影響見 §6）
