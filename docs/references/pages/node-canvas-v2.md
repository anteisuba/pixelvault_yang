...
13. **「连到镜头」弹层**（音频 / 图片 / 文本三类卡工具条上的那颗键）：顶行「新建镜头」（在右侧落视频卡并连线），下面按镜头带顺序列出画布上已有的视频卡（缩略 · 名 · 时长），点一张即连——音频连成音轨，图片再选首帧 / 尾帧，文本连成镜头说明；目标槽已有内容时显示「替换」而不是静默覆盖；连完目标卡短暂高亮并滚到可见。与拉线（自动落槽）、视频卡 + 菜单 / @ 三条路汇到同一份槽绑定。
...
- 工具条 `加语气 · 裁剪 · 转文字 · 连到镜头 · 下载 · ⋯`。**裁剪** = 卡下方的提示词栏换成裁剪面板（大波形 + 两端手柄 + 入 / 出点读数 + 只播选区的试听；I / O 设入出点；Esc 取消），「裁剪为新版本」非破坏地落进版本点，原音留作上一版；音频裁剪在客户端做（WebAudio 解码 → 按入出点切 → 编成 WAV → 走现有上传管线落成新版本），不扣积分；视频裁剪走剪辑台的渲染层（同一套手柄）。转文字 = ASR 转写后**派生一张文本节点并连线指回**（不写回本卡，转写中卡上走进度线）；生镜头 = 新建视频节点并把这段当音轨；⋯ = 改名 / 复制 / 拆出当前版本 / 归属角色… / **来源（只读）** / 删除。
...
- 参考轨 = 提示词栏首行，三组（图 · 视频 · 语音）以细分隔线分开，每项 32px 编号缩略（图角标 首 / 尾），每组末尾一个虚线加号 → 画布上的卡 › / 上传 / 从素材库（素材库按组过滤）。上限按模型能力表（满了加号灰、不藏）；组内序号就是 @ 的号：`@图1` / `@视频1` / `@语音1`，删一项后面顺位。展开态的画中框在播放器与说明之间摆同一条轨。
...
- 剪辑台：`src/components/business/node/edit-desk/**` · `src/hooks/node/use-edit-desk.ts` · `src/lib/edit-project.ts` · `src/constants/edit-desk.ts` · `src/constants/render-video.ts` · `src/services/video/render-video.service.ts` · `src/app/api/studio/render/**` · `workers/render-video/`
