...
- LoRA 库搜索已修复：提交新关键词立即清空旧结果，失败时显示错误；普通关键词搜索要求全部词项匹配，隔离旧规则的兜底快照，保留所选排序。浏览器验证「终末地」125 项、「从零开始的」126 项；详情底部恢复宽主按钮加两个图标按钮。全量 Vitest 600 文件通过（6210 项通过、1 项跳过），类型检查及相关文件 lint / 格式检查通过；未 push。
