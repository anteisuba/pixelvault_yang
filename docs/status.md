...
- **画布模块化重构 / Haivis 对标（2026-07-13，分支 `codex/canvas-modular-redesign` @ `a62bb1e1`）**：已落地 W0 分栏壳、CanvasSurface 外观（白/黑+多色、可自定义图）、散图纯图选中+四角缩放、对象工具条（命名/分类/展开/下载/快捷编辑/删除；AI 编辑在更多）、快捷编辑（元素提取 API+返回）、legacy `/studio/edit` 兼容跳转、卡匣与底栏并排、助手历史(localStorage)/模态 UI。连线/剪线已从底栏移除。**仍欠**：助手 DB/R2 跨设备会话与分享、共享 Assistant shell、指令式通用 edit API、镜头/档案纯图选中、CapabilityRuntime 完整抽取、S6 空态插画、导航侧栏 Edit 入口已撤。
