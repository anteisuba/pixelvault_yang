...
| **pre-push（husky）**             | `tsc --noEmit` → `npm run lint` → `npx vitest run --reporter=verbose`——**三关全过才能推**（合计约 8–9 分钟，不许 --no-verify 跳过） |
| CI `ci.yml`                       | push/PR 再跑一遍 tsc + lint + unit（见 cicd.md）                                                                                    |

- **全量纪律**：声称"绿"之前必须全量 vitest（~4.5min）——定向子集抓不住跨文件漂移；全量 tsc ~4min 后台跑 + 显式捕获 exit code（管道会吃退出码），禁止因超时跳过。
...
- Date: 2026-07-10 · Method: scripts / husky 钩子 / e2e spec 清点读取；时长数字（vitest ~4.5min、tsc ~4min）为经验口径。
