...
| 路由                                                            | 等级     | 375px 结构                                                                                                                                                                                                                                                                                                                                                                                                       |
| --------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/studio/image` `/video` `/audio` `/enhance` `/analyze` `/edit` | **降级** | 预览占满，参数面板进底部 vaul 抽屉，生成栏固定在 `safe-area-inset-bottom` 之上；模型选择只用 `layout="drill"`；助手 dock 改全屏 Sheet；`/studio/image` 落卡见 `../pages/studio-image-mobile-request.md`（owner 2026-09-03 拍板方向 A，本轮只验证图片模式）；`/studio/video` 落卡见 `../pages/studio-video-mobile-request.md`（owner 2026-09-03 拍板，复用图片端 composer 结构换视频 chip，队列条移动端可见优先） |
