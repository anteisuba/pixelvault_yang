...
**发送机制仍不变**：胶囊→@名字纯文本，closeup 随其 character 进 image_urls；`@特写N` 的图例映射（把「特写N=角色X面部」写进自动图例）随 v4 §7.2⑦「video 侧自动图例」一起接（当前 `buildShotReferenceLegend` 只服务 shot 图像节点，未接 video/Seedance 生成路径，仍是后续项）。测试：graph 1 跳收割 2 例、composer closeup 表面化/auto 名 2 例、strip 分组+＋特写+身份单元 caption 4 例、连线规则 2 例；各子片全量 tsc 我方文件型净（V2-4d 那轮全量 tsc 另有并行会话 KreaAssetBrowser.tsx 编辑中的无关报错，非本改动）。
