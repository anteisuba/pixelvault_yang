...
> ## ⚠ 行为事实框（2026-07-31 · 本文正文有三处已被实现推翻）
>
> 本文是**长篇施工图**，正文里下列三项**描述的是已退役的行为**。行为事实以 [`docs/status.md`](../../status.md) + 代码为准，**不要照正文施工**：
>
> | 正文怎么写                                                 | 今天的事实                                                                                                                                                          | 事实源                                                          |
> | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
...
房间语言五件套（2026-07-17 v3 补回墨线）：**场记卡**（节点）· **吞噬**（绑定手势）· **墨线**（关系台账：骨干常显/成分选显，brand-dna 房间表本有）· **盖章**（状态）· **剧本笺**（助手）。
...
| 助手 dock          | `StudioNodeAssistantDock.tsx` + `AssistantConversation.tsx` | Haivis 对齐：默认窄态改全高稳定分栏、低 chrome 纸笺皮肤、共享 assistant shell/header/composer；收起让画布真实扩宽；ScriptDoc 展开能力保留但不得把 820px 直接作为窄视口 grid 列（见 §3.1）。**操作员化 C1（2026-09-02）**：画布宿主 `hooks/node/use-canvas-operator-host.ts` 已在 `StudioNodeWorkbench` 经 `<StudioOperatorHostProvider>` 挂上（未接 UI，dock 仍走 marker 链到 C2）；落笔纯函数 `lib/canvas-operator-apply.ts`（`(graph, step, aliases) → { patch, inverse, aliases }`，零 React、钱闸 ② 源码锁），快照 `lib/canvas-operator-snapshot.ts`（根上无 `prompt`，`modelOptions` 只列画布上出现的 nodeType、相对价签 `1×/2.2×/free`）；`useNodeWorkflow` 加 `readState / applyGraphPatch / readUndoTarget`；批撤（建 / 连）只在仍是撤销栈顶时可点且走 `undo()`，`set_*` 走字段级逆补丁；节点 data 新增可选 `assistantPrimed`（`prime_node_generate` 的生成键 primed 态，C2 接视觉） |
...
对标证据与 CSS 实测见 Haivis 画布拆解（已随 ui-inspiration 清理删除，见 git 历史）。本页采用其**空间纪律、CSS 比例和助手交互**，不复制白板品牌外观。

1. **默认助手 = 稳定分栏**：画布 `minmax(0,1fr)` + 窄助手列；助手收起时列宽动画归零、DOM 保持 mounted、React Flow 获得真实可视宽度。当前浮动 overlay + 重阴影/backdrop blur 退场。
2. **响应策略**：画布内容宽度足够容纳“至少一个 400px 节点 + 360px 助手”时默认分栏；不足时默认收起，打开走窄 overlay；移动端走 `ResponsiveDialog`/drawer。用容器可用宽度判定，不仅看全局 viewport。
3. **ScriptDoc 宽态**：现有 expanded 820px 双栏不直接占 grid 列；宽屏可进入扩展 workspace，窄屏用全屏/大面板。默认 360px 助手和 ScriptDoc 重编辑是两种披露层。
4. **共享 shell，不共享业务**：Node 与 Studio Image 提取 `AssistantDockFrame` / `AssistantHeader` / `AssistantComposerShell` / `AssistantMessageViewport` 类视觉原语；Node 保留流式消息、节点 references、route、research、ScriptDoc、应用前询问，Studio 保留 prompt output、参考图、语言、enhance route 与填入/追加/复制。
5. **助手 header**：空间结构对齐“会话名 + 新建 / 历史 / 分享 / 收起”。现有仅“新建”可直接接；历史需要会话持久化，分享需要只读链接/权限契约，未落地前不画死按钮。
6. **composer**：引用 chip + 输入区 + 左侧上下文/workflow + 右侧推理/route/发送；复用 `ui/prompt-input.tsx` 的 IME/触屏 focus compound primitive。Node 当前 text-only，图片 chip 的“可见选择”与“模型真正看图/执行编辑”必须拆片。
7. **命中区**：桌面 fine pointer `compact=32px` / `default=36px`，触屏 coarse pointer `44px`，绝对底线 24px；控制条壳保持 44px。视觉 glyph 可小，不再强制所有桌面按钮 44px。
8. **布局联动**：改分栏必须同时处理 `StudioNodeWorkbench` 的 `bottomRowInsetPx`、空态 `md:right-[30rem]`、CastDock、CanvasBottomDock 与 React Flow resize；禁止只换 dock CSS。
9. **会话正确性先于历史 UI**：Node 先补请求侧 16 条 replay window、retry 不重复 user turn、稳定 `projectId` 隔离；Studio prompt assistant 按 image/video/audio modality 隔离。四项未绿前不接历史持久化。
...
⚠ **「吃」的两条边（特写 → 角色卡 · 音色 → 角色卡）2026-08-10 起不画线、卡也没有入端口**（owner 拍板，判据与两类边的证据落点见 [`canvas-identity-card.md` §3](canvas-identity-card.md)）。**这不改食物链** —— 边照建、收割照读，变的只是**建它的手势**：从「拉线」换成「拖节点进卡」。⛔ 别据本节给卡补一个入端口。
...
| S4 ✅ 2026-07-10        | Chrome：failed 真红收尾 + 顶栏片名条 / 工具条核查 / minimap 蓝图（剧本笺皮肤裁出挂账，见 node-canvas-s4-chrome.md（已删，见 git 历史））                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 中                                                                            |
| S5a ✅ 2026-07-10       | Cast 卡匣四分区（**镜像视图版**：身份节点暂留画布，卡匣为第二呈现；点卡聚焦+分区新建+可折叠。「迁入/退场+添加菜单收敛」移交 S5b，任务包 node-canvas-s5a-castdock.md（已删，见 git 历史））                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 中                                                                            |
| S5b B0+B1 ✅ 2026-07-10 | 卡匣形态 v2（把手+浮层 tabs）+ 吞噬核心（拖拽入腹三拍 + 咬不动 + 胃取出 + 身份节点/连线渲染退场 + 添加菜单收敛）。B2 增强三件套（磁吸/快投/张口预览）留二次会话，任务包 node-canvas-s5b-ingest.md（已删，见 git 历史）                                                                                                                                                                                                                                                                                                                                                                                                                       | 中高（新手势 + 契约校验可视化，触屏点选兜底必测）                             |
| S5c ✅ 2026-07-10       | 浮层修缮（列数自适应/裁切根因修复）+ 角色档案卡（紧凑卡徽章 + 档案面板：视觉/身份/听觉/出演四区）+ 散图一等公民 + 融合/拆出循环（含本地文件拖入），任务包 node-canvas-s5c-dossier.md（已删，见 git 历史）                                                                                                                                                                                                                                                                                                                                                                                                                                    | 中（散图↔卡匣是新方向手势，`referenceAssets`/`fusedIntoNodeId` 向后兼容增量） |
| S5d ✅ 2026-07-11       | 业务模型 v3.1 对齐修正六条（§6.0 清单：①横匣回滚 ②隐藏条件=有下游引用 ③upload-first+分类系统+分类进图例+frame 退役 ④收集器档案卡面 ⑤融合目标改画布 ⑥卡匣再喂）+ owner 追加拍板（卡匣只放收集器卡、音色/参考视频=素材回添加菜单）+ 两缺陷修复（融合三拍补齐/把手双锚点），任务包 node-canvas-s5d-model-align.md（已删，见 git 历史），偏差回写 §9c                                                                                                                                                                                                                                                                                            | 中（渲染层隐藏条件翻转 + schema 兼容增量）                                    |
| S5f-A ✅ 2026-07-11     | 画布实体拖拽吞噬全覆盖（收集器卡/音色/参考视频/散图 → 合法目标；散图→镜头图按矩阵正确拒绝），任务包 node-canvas-s5f-gestures.md（已删，见 git 历史）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 中（onNodeDragStop 扩面复用 evaluateCastIngest）                              |
...
- **画布添加菜单的收集入口（2026-08-05 owner 真机拍板）**：`CanvasAddMenu` 的组织分组只显示一个「收集」，点击直接创建收集器，不再展开「角色收集 / 场景收集」二级项。当前创建路径复用 `organize.character` 作为兼容事实源；`organize.scene` 仍保留给存量项目与助手意图解析，不从该手工菜单暴露。
...
- 交互 canon（D1–D5 修订版）：中键平移 / 参数在节点 / 右栏纯助手 / 两级切换器 /（原绿）生成键例外 → 改述为石绿；**D3 连线部分被吞噬拍板取代**（2026-07-10）；**2026-07-17 v3 再修订：连线两级回归为关系台账**（手势仍吞噬），合法性矩阵 `node-connection-rules.ts` 完整保留 = 吞噬合法性事实源，两级判定是渲染层查表不得反向影响合法性。
