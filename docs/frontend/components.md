...
<ArenaForm
  isCreating={false}
  onBattle={(input: StartBattleInput) => {}}
/>
```
...
const {
  ...
  referenceImage, isDragging, fileInputRef,
  handleDrop, handleDragOver, handleDragLeave,
  openFilePicker, handleInputChange, clearImage,
  // 提示词增强（委托自 usePromptEnhance）
  isEnhancing, enhanced, enhancedOriginal, enhancedStyle,
  enhancePrompt, clearEnhancement, applyEnhancedPrompt,
  // 工具
  resetForm,
...
const {
  ...
  create, update, remove, verify, refresh,
} = useApiKeys()
```
...
type ProviderCapability =
  | 'negativePrompt' | 'guidanceScale' | 'steps' | 'seed'
  | 'referenceStrength' | 'quality' | 'background' | 'style'
...
### API
```ts
import { hasCapability, getCapabilityConfig, ADAPTER_CAPABILITIES } from '@/constants/provider-capabilities'
