...
- ~~`use-video-composer.ts` 的槽位徽标（图N）没接选帧函数：要重排一个两千行的 hook~~ → **两处都说错了，2026-08-08 已核**：
  1. **那个徽标不存在。** `imageSlotIndex` / `audioSlotIndex` / `videoSlotIndex` 三个字段**零消费者**——`grep 'SlotIndex' src/**/*.tsx` 只命中测试里的类型 stub，没有任何组件渲染「图N / 音N 角标」。三个字段连同注释已删。⚠ 与 memory 里那条「退役 UI 闸前先确认组件真有人渲染」同形，**我差点去修一个幽灵**。
  2. **难度也说大了。** `effectivePreviewModel` 只依赖 `data.model` 和 `options`（159 行，在 `referenceTokens` 之前），上移是几十行的事，不是「重排两千行 hook」。这个理由不成立——好在结论不变（该删不该接）。

  ⚠ `payloadImageUrls` 与 `slotIndex` **留着**：它们还喂 `autoName(kind, slotIndex)`，「角色1」「镜头2」这种自动名字是用户可见的、并且会变成 `@token`。名字里的数字**不必**等于真实载荷下标——发送时 `filterReferencedImages` 按真实候选表重新解析「名字 → 位置」，那才是权威。

- ~~**两套容量源**本身没统一~~ → ✅ **已统一**（见 §9.12）。
- 未真机实测（改动集中在发送路径，画布上无可见变化；两侧共 8 + 22 条单测覆盖）。

### 9.12 ✅ 画布视频域剩余七条清空（2026-08-08）

owner「这个 chat 把这些解决就行了」。七条里**前四条共用一个根因**，合成一刀做的。

#### 根因：容量算了两遍

预览层按 `contract.slots`、发送路径按 `getMaxReferenceImages`（视频与音频甚至是写死的 `.slice(0, 3)`）。后果不是「数字对不上」这么轻：**多图参考档的 `slots.videos` 是 0，发送路径照发三条视频** → 火山 400，而预览早就显示「这段视频不会发送」。界面说的和发出去的是两回事，正是 §8.7 第 1 条警告的那件事。

`lib/node-video-send-slots.ts`（纯函数 + 6 条测试）两边共用。跨模态总额按**实际采用数**扣，不按候选数——拿候选扣会让「本来就发不出去的那几条」把图片的位置也吃掉。

#### 形态与提示（§8.6 / §8.7）

- 添加入口按模式收窄：关键帧与多图参考**不渲染**音频/视频组（不是置灰）。判据取自同一个契约，不另立表。
- 关键帧档的图片区是**两个具名位置**：token 的名字取自节点自己的 `imageCategory` → 面板里是「首帧」「尾帧」两行。
- 已连着但当前模式发不出去的素材，复用既有的行内「不会发送」徽标。⚠ 与 `imageOverflow` **分开传**：那个是容量溢出、驱动面板顶部的容量计数，混进去容量数字就开始说谎。

#### 命名统一（§2，三语）

「关键帧首/尾」→「**首帧/尾帧**」（i18n 与**发给模型的图例常量**一起改）· zh 的 `refKind.keyframe`「首尾帧」→「关键帧」（en/ja 本来就是 Keyframe）· `videoMerge` 三语都不一致 → 统一「合成」。

改法：目标串**全部从已有键里拼**，不手打 CJK；改完对 HEAD 做逐叶子键 diff——三份各 4596 叶子键，各只变 3–4 个，键集完全不变。

⚠ **`imageShot`（镜头图）vs `refKind.shot`（镜头）没动**：§2.2 没给处置意见，而且两者指的本来就不是一回事（节点类型 vs 引用类别）。

#### HappyHorse：标签旧了，不是 slug 打错

目录注释明写「v1.1 是直接后继，所以 enum id 保持不动」，`externalModelId` 也是 v1.1。所以 `MODEL_VARIANTS` 的 `happyhorse-1.1` 是**对的**，错的是给用户看的标签（实际调 v1.1 却写 1.0）。

#### 第三层单价

只在**渠道层**露出（同型号不同渠道价差很大，放型号层不知道说的是哪家），落点是**悬停层**而不是常驻副行——第三层副行是故意空的（owner：「只留公司名字」）。无数据则隐藏。⚠ 用按 string 查的薄封装，不断言成 `AI_MODELS`：目录 DB-first，运行时会有代码里没有的 id。

#### 首页价格（owner 拍板见 §9.7）

Seedance 2.0 `$0.1 → $0.3034`（低报 3 倍）· Kling V3 Pro `$0.3 → $0.168`（原来是**高**报）· Veo 3.1 `$0.2 → $0.4`。
