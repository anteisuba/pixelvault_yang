 ...
 */
type AssistantOperatorAskEvent = Extract<
  AssistantOperatorEvent,
  { type: typeof ASSISTANT_OPERATOR_EVENTS.ask }
>

function normalizeAsk(
  ask: NonNullable<AssistantOperatorTurn['ask']>,
  askId: string,
): AssistantOperatorAskEvent | null {
...
export type OperatorTurnDecision =
  ...
  /**
   * 反问（C3）。**排在 `tool` 前面**：模型同一轮既问又做时，问题才是它真正的
   * 意思。事件在这里就归一好了 —— `normalizeAsk` 判空的那条规则（问题是空串
   * 就当这一轮没有 ask，继续按 tool / message 走）留在下面的收敛函数里，
   * 两条路共用同一份。
   */
  | { kind: 'ask'; ask: AssistantOperatorAskEvent }
...
- The ONE exception is asking: when the rules above say to ask instead of acting, call no tool and reply with ONLY the {"ask":{…}} JSON object described earlier, nothing else around it.`

/**
 * 原生路上认出「这条正文其实是一次反问」的记号。
 *
 * ⚠ 只是一道**便宜的门**，不是解析器：命中了才去跑 JSON 路那份 `parseTurnJson`。
 * 不命中就一个字符串都不解 —— 否则每一句正常的话都要白解一次 JSON。
 */
const OPERATOR_NATIVE_ASK_MARKER = '"ask"'
...
/**
 * JSON 路解出来的那个对象 → 收敛后的决定。**两条路共用**（原生路在正文里读出
 * 同一个形状时也走这里），所以 `ask` 优先于 `tool` 这条规则只有一份。
 */
function toOperatorTurnResult(
  ...
  if (turn.ask) {
    ...
    // ⚠ 归一化吐 null = 问题本身是空的，那不是一次反问：照常走 tool / message。
    if (ask) {
      return {
        ...plan,
        ...(turn.message?.trim() ? { message: turn.message.trim() } : {}),
        decision: { kind: 'ask', ask },
      }
    }
  }
...
async function requestOperatorTurn({
  ...
  const shared = {
    ...
    contextCompactionTargetLength: OPERATOR_CONTEXT_COMPACTION_TARGET_LENGTH,
    modelId: run.modelId,
    // 取消信号两条路一视同仁（C0-c）：用户点了停，在飞的 provider 请求跟着停。
    ...(run.signal ? { signal: run.signal } : {}),
  }
  ...
  if (
     ...
     * 合并时必做 ②：原生路的反问通道。
     *
     * OpenAI / Gemini 的 tool call 里没有「我要问你一句」这一档 —— 加一个伪工具
     * `ask` 会进域工具表，而域工具表正是 JSON 路提示词的来源（改一个字，慢路的
     * 请求体就不再逐字不变，那是本片的验收条件）。所以反问走**正文**：提示补丁
     * 里让模型此时只回那个 `{"ask":…}` 对象，这里认出来后交给 JSON 路那份
    ...
    if (nativeText.includes(OPERATOR_NATIVE_ASK_MARKER)) {
      ...
      if (asked?.ask) {
        const converged = toOperatorTurnResult(asked, run)
        if (converged.decision.kind === 'ask') return converged
      }
    }
  ...
  return toOperatorTurnResult(turn, run)
}
...
export async function* runAssistantOperator(
  ...
  try {
    for (let index = 0; index < LIMITS.maxSteps; index += 1) {
       ...
       * 才是它真正的意思（它不知道答案就动手，正是这条协议要拦的）。优先级已经
       * 落在 `toOperatorTurnResult` 里，两条路共用一份。这条流到此结束，答案由
       * 客户端作为下一条 user 消息带 `answeredAskId` 重发 —— 与 `confirm_request`
       * 复用同一条「无服务端会话态」的机制。
       */
