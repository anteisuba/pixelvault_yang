...
/**
 * **结论一行的回落**（§9.1 ④）——印证最多、层级最高的那一条怎么说。
 *
 * ⚠ 2026-09-12 起它是**回落**而不是正路：卡上那一句改由工具环收尾时一次结构化
 * LLM 往返**归纳**出来（`synthesizeResearchConclusion`）。理由是实测里这份摘录
 * 直接把一段知乎评论的原句摆成了本轮的「事实」—— 摘录读起来像结论，却是某个人
 * 说话的半句。归纳那一跳解不出来时照旧用这一份：结论栏有一句话，⛔ 不留空。
