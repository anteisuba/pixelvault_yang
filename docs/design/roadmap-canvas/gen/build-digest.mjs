  ...
  [
    ...
    table(
      ...
      [
        ...
        ['—', '拆分与反推', '把用户的话拆成 NAI 标签；参考图反推成自然语言或标签，尽量复刻（09-24 owner 提出）；④ 画板在第 6 页，施工完实跑 X1–X8 通过（b8300847）', '待上线'],
...
const PROGRESS = {
  ...
  blocks: [
    ...
    ptable([
      ...
      P('—', '拆分与反推：把用户的话拆成 NAI 标签；参考图反推成自然语言或标签，尽量复刻（09-24 owner 提出）；9f6f8a22 · b8300847', '—', '待上线'),
...
const SPLIT_REVERSE = {
  ...
  heading: '拆分与反推：全状态',
  sub: '⚠ 09-24 已施工（9f6f8a22 · b8300847），实跑 X1–X8 通过；实跑后补：查不到的先退回改写一次、「相近」只认形态差异、NAI 拦 @ImageN 与整句、换台型号跟着换台、NAI 能改比例。owner 09-24 ① 七问全选最简：不设模式、不加入口、不列对照、自动核对、只要文字给复制块、孤立反推面板删掉；追问定「NAI V5 + 换场景」当场问一次。新画面只有两样：**代码块的复制键**和 **X7 那一题**，其余全部沿用「A 对话流：全状态」。场景用同一张测试图。',
  blocks: [
