...
- `docs/plans/lora-generate-bench-redesign-2026-07.md`（已退役）
- `docs/plans/lora-generate-claude-draft-2026-07.md`（未确认草稿）
...
- ⬜ 后续（owner 未拍板 / 需真机）：**助手移动端近全屏 sheet**（`LoraAssistantDock` 现移动端 `return null`，FAB 打开的 sheet 是独立新组件，本轮未做）；移动端参数/负面折叠、软键盘避让、结果大图 sheet 等细节，owner 真机后再收。
