...
describe('buildVideoSendPreview (R3-6b §2 发送图例预览)', () => {
  ...
  it('keeps Gemini image-only and allows the migrated execution route', () => {
    const nodes = [
      makeNode('char1', NODE_TYPE_IDS.characterImage, {
        mediaUrl: 'https://cdn/char.png',
      }),
    ...
    expect(preview.request.audioUrls).toBeUndefined()
    expect(preview.blockers).not.toContain('execution-not-migrated')
...
/* ═════════════════════════════════════════════════════════════════════════
 * v4 分支（接线清单 5）—— 预览读的是 `buildV4VideoPayload` 的那一份装配结果
 * ═══════════════════════════════════════════════════════════════════════ */

const V4_NOW = '2026-09-07T00:00:00.000Z'
