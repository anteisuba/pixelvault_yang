...
> ⚠ **2026-09-06 owner 定：第三期画布按 [`node-canvas-v2.md`](node-canvas-v2.md) 重做，本文只描述现状。**
> 目标态（四类节点 + 子型 · 具名槽端口与合法矩阵 · 槽内版本轮播 · 稳定命名与带边快照 · 助手 op 集补全 · 镜头带时间轴 · v4 迁移 · **皮肤并入全站脊柱、`--canvas-*` 令牌世界整体删除**）一律以 v2 为准。
> 本文继续有效的只有一件事：**它是今天代码长什么样的记录**——在飞回归、定位现有实现、读懂旧决策为什么这么定，都还看它。⛔ 不要拿本文的外观数值、纸卡材质或 `--canvas-*` token 去施工新东西。本文不删，第三期落地后与 v2 合并。
