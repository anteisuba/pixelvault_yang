...
- `/studio/node`（画布：现状见 `../pages/node-canvas.md`，第三期目标态见 `../pages/node-canvas-v2.md`）· `/studio/lora` · `/studio/3d`（搁置）· `/studio/edit(/**)` · `/studio/enhance` / `analyze`（工具路由）。
...
- 助手宿主 = 右侧 dock（**现行施工基准见 `../pages/assistant-shell.md`**——2026-09-06 owner 定方向 C「工作日志」：覆盖式面板 inset 24 / 默认 560 · 收起态 48px 图标轨 · 顶部进度带 · 三档确认 · 计划卡 · @ 看图 · 助手设置 persona；画布第三期见 `../pages/node-canvas-v2.md`）。
...
| 路由                                                            | 等级     | 375px 结构                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/studio/image` `/video` `/audio` `/enhance` `/analyze` `/edit` | **降级** | 预览占满，参数面板进底部 vaul 抽屉，生成栏固定在 `safe-area-inset-bottom` 之上；模型选择只用 `layout="drill"`；**助手移动端下一轮，当前 `isMobile → return null`（owner 2026-09-06 定；落点 `StudioOperatorDock.tsx:320`，⛔ 原「助手 dock 改全屏 Sheet」作废）**；`/studio/image` 落卡见 `../pages/studio-image-mobile-request.md`（owner 2026-09-03 拍板方向 A，本轮只验证图片模式）；`/studio/video` 落卡见 `../pages/studio-video-mobile-request.md`（owner 2026-09-03 拍板，复用图片端 composer 结构换视频 chip，队列条移动端可见优先） |
| `/studio/node`                                                  | **降级** | 见 `../pages/node-canvas-v2.md` §0.1 移动端节（第三期非目标，另行排期）                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- 2026-09-06 · 助手改版落卡：指针改指 `../pages/assistant-shell.md`（方向 C 施工基准）；移动端助手由「改全屏 Sheet」改为「下一轮，当前不渲染」；画布补 `../pages/node-canvas-v2.md` 目标态指针。只改文档。
