...
图片域的 `analyze_references` 专门分析已挂载参考图：将未分析的图片一起送入视觉调用，分别记录身份与服装、姿势与接触关系、比例/轮廓/明暗/材质/色彩/光照、场景和不确定项，不评价参考图是否符合待生成目标。视觉记录以素材 URL 关联，随助手历史保存；图片未变化时复用，换序按当前列表重新绑定，换图只补分析新图。每轮根据最新要求整理来源分工、保留项、排除项和创作要求；助手在折叠操作日志外展示分工卡，历史卡保留当时的来源与分工。读取源图遇到 HTTP 错误时，该步骤标为失败并把当前图号交给助手说明，要求重新上传或替换；不跳过失效图接受不完整分析，不把整轮变成无上下文的通用报错。

带参考图的 `set_prompt` 必须先取得当前顺序的完整分析；分工有未解歧义时先询问。写入前检查完整最终提示词（含追加后的原文）的语义冲突，返回具体遗漏、错配或改变创作要求的补充；两次检查失败后停止继续付费检查；同一轮通过检查并写入一次后，拒绝后续重复改写，新一轮用户指令可继续修改。程序同时检查图片引用是否存在。`critique_result` 拒绝把已挂载的源图当成失败成图评价；实际成图评价同时接收结果图与当前参考图进行对照。以上约束用于减少提示词偏离，不保证生成模型逐次遵从。
