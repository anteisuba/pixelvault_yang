...
| CAN-R03 | Ingest       | ~~默认不显示连线~~ → **两级墨线**（v3 拍板 2026-07-17：骨干边常显中性墨/成分边选中显石绿细线，`plans/canvas-relationship-v3-2026-07.md` §2）；无效连接/剪线工具已移除 | 首期（R3-1）              |
| CAN-R04 | Ingest       | 兼容拖拽时临时端口与指针附近拒绝原因                                                                                                                                  | 首期                      |
| CAN-R05 | Relationship | 只看选中关系/诊断关系 → 并入两级墨线（选中显现 + 「关系线」双态开关）                                                                                                 | **首期**（v3 提级，R3-1） |
