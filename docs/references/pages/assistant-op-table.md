...
| `set_model`             | 图视 | 改       | `modelId`（只能取快照 `availableModels`）                                             | `{ modelId }` 可为 `null`                       | free · 可逆   | `model` 模型 chip                        | 无关               |
| `set_specs`             | 图   | 改       | `aspectRatio` + `resolution` **两个必填** · `quality?` `preview?` `background?`       | 旧五格（每格可 `null`）                         | free · 可逆   | `specs` 比例 chip + 高级参数             | **缺口**           |
| `set_video_specs`       | 视   | 改       | `durationSeconds?` · `aspectRatio?` · `resolution?`（逐型号有无）                     | 旧三格**永远带齐**（缺的那格 `null`）           | free · 可逆   | `specs` 比例 / 时长 / 清晰度             | **缺口**           |
...
| ~~`set_capability`~~    | —    | （改）   | **不存在**。画板 D7 那张骨架里有一行 `key · value（11 派生的专属 chip）`              | 旧值                                            | free · 可逆   | 专属 chip 行                             | **缺口 · 见 §6-1** |
...
LoRA 装配台的 op = **通用件里的那些**（与工作台同表，见 §2 的「图视」行，`set_specs` / `set_count` / `request_generation` 三条除外）+ 下面这八条域专属。
...
| **闪**               | 被改对象 outline 闪一次，**320ms**，只动 `outline` 与 `opacity`（⛔ 不动 transform / 尺寸：被改的东西常在视线里，动尺寸会让整片重排）                      |
| **motion-reduce**    | `prefers-reduced-motion` 下降到 **1ms 并保留终态**，与画布其余三档同一条规矩                                                                               |
| **既有例外**         | 画布的 **`add_node` 不闪** —— 新 id 由执行器现铸，助手这一侧根本看不见它；一张卡凭空出现本来就比闪一下更响                                                 |
...
| #   | 差距                                                                                                                                                                                                                         | 谁做                                           |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| 1   | **工作台没有「专属 chip 值」op**：画板骨架里的 `set_capability` 在代码里零命中，11 的专属 chip 行助手够不着                                                                                                                  | 进度表 11 + 21                                 |
| 2   | **没有 `unmount_reference`**：画板写的是 `mount_reference / unmount`，代码只有挂没有摘（LoRA 有 `unmount_lora`，参考轨没有）                                                                                                 | 进度表 21                                      |
| 3   | **`set_model` 不带渠道**：画板骨架是 `modelId · channelId?`，代码载荷只有 `modelId` + `modelLabel`，换渠道助手做不到                                                                                                         | 进度表 21                                      |
| 4   | **工作台字段不闪**：320ms outline 闪只接在画布（`canvas.css` 的 `.node-assistant-touched`），Q4 A 的「字段 outline 闪一次」在工作台 / LoRA 上只有回执行没有闪                                                                | 进度表 21 + 22                                 |
...
- 回执与闪：`src/components/business/studio/assistant-operator/StudioOperatorCheckpointCard.tsx` · `src/constants/motion.ts`（`ASSISTANT_TOUCH_FLASH_MOTION`）· `src/app/canvas.css`（`.node-assistant-touched`）· `src/hooks/node/node-ingest-dom.ts`
...
- 与画板 D7 ② 那张 12 行骨架的出入已逐条记在 §6：`set_capability` / `unmount` / `channelId` 三条骨架有而代码无；`delete` 骨架写「不可逆」而代码给了 `inverse: add_node`（confirm 档、进 `CANVAS_APPLY_OP_IDS`）；`plan_rerun_downstream` 骨架与 `generate` 并列在「paid · 确认卡」而代码是 read / free；骨架的「现有 `NODE_ASSISTANT_AUTO_APPLY_OPS` 8 条」是 v3 残留，现行是 v4 的 31 条。
