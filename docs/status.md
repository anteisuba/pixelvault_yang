...
- 图片助手新增独立参考图分析、多图视觉输入和来源分工卡：按素材 URL 保存视觉依据，换序重新绑定编号，换图只补分析新图，刷新后可复用；明确保留/排除项与创作要求。有参考图的提示词写入需完整分析，并检查完整提示词的引用与语义冲突，最多两次失败检查；同轮通过后只写入一次，阻止成功后的重复改写。源图不作为失败成图评价，成图点评同时对照参考图。读取源图遇到 HTTP 错误时明确图号并结束该步骤。四图实测已确认原图2返回404，修复后正常提示重新上传；其余三图的分工卡、刷新保存、缓存复用及确认后提示词写入通过桌面/390px 验证。全量回归基线为659文件、7064项通过（1项跳过）；末轮修复另通过287项定向回归，类型检查、目标lint无错误（3条已有测试警告）。独立暂存版本328项定向测试通过；类型检查报错与提交前基线一致，无新增错误。原图2需重新上传才能继续完整四图验收；此前成图偏二维与素材自动导入失败未通过本轮出图验证。本轮未生成图片，已本地提交，未推送/发布。
...
- LoRA 搜索与收藏详情：搜索恢复宽松匹配，提交关键词自动切到「推荐」（相关性优先），仍可手动选择其他排序；保留新搜索清空旧列表的修复，隔离严格规则的旧快照。收藏卡片点击封面可打开共享详情抽屉，展示已保存的封面、触发词、底模和样图，保留生成与来源入口。842px / 375px 浏览器实测详情通过；全量 Vitest 600 文件通过（6211 项通过、1 项跳过），类型检查及相关文件 lint / 格式检查通过。官网结果数量因浏览器访问策略限制未重新核对。本轮未 push。
