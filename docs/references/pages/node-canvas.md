...
| 顶栏               | `CanvasTopBar.tsx`                                          | 项目名改「片名条」：纸色小条 + 炭字；默认模型 chip / 添加节点结构不变（添加节点主钮可用纸色反相丸）                            |
| 助手 dock          | `StudioNodeAssistantDock.tsx` + `AssistantConversation.tsx` | 「剧本笺」皮肤：纸面、顶部签头、气泡 `paper-soft`；三态/ScriptDoc 工作区结构不动                                               |
| 工具条             | `CanvasBottomDock.tsx`                                      | 深炭 chrome 微调，不改交互                                                                                                     |
| minimap            | `CanvasMiniMap.tsx`                                         | 蓝图纸：深青底 `#101820` + 青线 `#2e4a5e`（domain 内硬编码可接受，或加 `--node-blueprint-*` token）                            |
...
| -------- | --------- | ------------------------------ | ---------------- |
| idle     | 无章      | 素卡                           | —                |
| queued   | 待拍      | 炭墨描边章，倾 2–3°            | 中性             |
| running  | 拍摄中    | 石绿章 + 深窗内进度墨条        | 进行中靠动效     |
| done     | 已收      | 炭墨章                         | 完成克制         |
| failed   | NG        | 红章（`--node-danger` 真红）   | 错误才红         |
| 审阅循环 | 过 / 重拍 | 章式按钮（approve = 盖「过」） | Board/审阅期接入 |

章 = 结构表达：`border 1px` 描边 + 同色文字 + `rotate(-2~-3deg)`，无纹理无阴影。
...
| S3               | 状态章系统（NodeStatusBadge → 章视觉）                                                  | 低                                                |
| S4               | Chrome：顶栏片名条 / 工具条 / minimap 蓝图 / 剧本笺皮肤                                 | 中                                                |
