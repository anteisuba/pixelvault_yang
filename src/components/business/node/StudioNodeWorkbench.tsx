...
function StudioNodeCanvas() {
  ...
  /**
   * v4 节点 → ReactFlow 节点：`type` 就是 `kind`（v4 注册表的键）。
   *
   * ⚠ **`measured` 必须留在这一层**：ReactFlow 受控模式下，节点尺寸是库通过
   * `onNodesChange` 的 `dimensions` 变更回传的，落不回节点对象就永远
   * `visibility: hidden` —— 卡在 DOM 里、量得到宽高、屏幕上一个都看不见（实拍
   * 过这一幕）。所以 RF 节点是一份**自己的 state**，`data` 跟着 v4 state 走，
   * `position` / `measured` 由 `applyNodeChanges` 维护。
   */
  const [v4Rendered, setV4Rendered] = useState<{
    ...
    setV4Rendered({
      ...
      nodes: (v4RenderState?.nodes ?? []).map((node) => {
        ...
        return {
          ...
          id: node.id,
          type: node.data.kind,
          // 拖过的位置留在渲染层：预览态的槽操作不该把卡弹回原位。
          position: before?.position ?? node.position,
          data: node.data,
          selected: before?.selected ?? node.selected ?? false,
