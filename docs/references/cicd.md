...
| Workflow                | 触发                                                                                                                               | 内容                                                                                                                                                                                                                                                                                                                   |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`                | push main/feat/\* · PR→main                                                                                                        | **四个 job**：lint（prisma generate → `tsc --noEmit` → eslint）· test（unit，`vitest run --reporter=verbose`）· audit（`npm audit` 容忍失败 + Prisma migration drift check，带 Postgres 16 service）· build（生产构建，needs lint+test，限时 10min）；Node 22 + npm cache，lint/audit 限时 5min，test/build 限时 10min |
...
- Date: 2026-07-13 · Method: 核对 main push run `29200125049` 与历史成功 run `29192757854`；Unit Tests 总耗时已接近 5min 上限，因此 test job 超时提升为 10min。`models:check-docs` 基线已补（见上）且脚本存在 0 模型解析退化（经 git ls-tree + 实测快照核验）。
