...
> 每项必须附证据，不接受"已确认"三个字。默认值与配方在 `references/ui-defaults.md`；日常任务的输入是 `templates/ui-request.md` 需求卡。改版级任务在这 8 项之外还要过 `scenes/ui-page.md` 的确认门（域定义 → 方向 → 关键切片 → owner 选向）。
...
grep -rn "^:root" src/app --include='*.css' | grep -v -e globals.css -e canvas.css
```

（第 1 条允许 `src/app/*.css`；第 4 条域 token 只写域根，`canvas.css` 因 Radix 传送门挂 `:root` 是已知例外。规则化进 eslint 是独立任务。）
