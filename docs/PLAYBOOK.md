...
**1 · 重构/升级一个页面 UI（改版级）**
开口："把 XX 页按工坊宅邸改版" / "XX 页太普通了，重做"。
我先做：该页**工作流调研**（先懂这页干嘛）→ 出 `references/pages/<页>.md` 施工图草案 + 概念稿（图）给你拍板 → Fable 定稿、Sonnet 实施。
流程：`scenes/ui-page.md` ｜ 你拍：方向与概念稿 ｜ 验收：`checklists/ui.md` P0。

**2 · 小 UI 调整（按钮/文案/间距，非改版）**
开口："XX 按钮太挤" / "这文案改成…"。
我先做：确认该页现行体系（v1 / 已改版，禁混搭）→ 直接改 → 机械检查 + 相关交互实跑。
...
调查 / 顶层设计 / 施工图 / 评审 → **Fable**；照施工图执行 → **Sonnet**（task chip 或新会话）；service/后端大活也可走 Codex（task packet 交接）。混合任务拆两半。
