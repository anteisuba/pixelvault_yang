# 画布 v3 实现记录 — node-canvas.md「导演的制片桌」

> ⚠ **本文描述的是已被删除的 v3 画布**（`StudioNodeWorkbench` · `NodeDetailPanel` · 12 个 legacy 节点类型 · 纸卡材质 · `--canvas-*` 令牌世界）。**现行基准是 [`node-canvas-v2.md`](node-canvas-v2.md)**，一切施工、设计与回归都看那一份。
> 本文只保留一个用途：**读懂旧决策为什么那么定**——代码里仍有若干注释按 `node-canvas.md §N` 引用本文的手势与色值论据（吞噬三拍曲线、盖章状态、边的族色落点）。⛔ 不要拿本文的任何结构、组件名、外观数值或 token 去施工。
> ⛔ 本文不再更新。它引用的组件多数已删，`文件:行号` 一律作废。
