...
- [ ] **列变更走 expand-contract**：加列直接来；删列 / 改列义 / 改类型留兼容期（先双写、后清理）。理由是部署顺序——`vercel.json` 的 buildCommand 是 `prisma migrate deploy && next build`，迁移先于新代码上线，旧 serverless 实例还在跑旧查询。⚠ 这**只约束数据库列**；代码层的调用方一次全改完，不留垫片（CLAUDE.md Engineering Principle 1）
