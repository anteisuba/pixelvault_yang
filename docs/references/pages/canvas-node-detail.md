...
> **Last verified**：2026-08-04（方向 E 已全量实装，见 §14.1）
...
| ~~6~~ | ~~四族的下游反查原型未覆盖~~ —— **已实现**（`useDownstreamUses`，十族共用，各族空态文案不同）                                                                                             | ✅ 2026-08-04  |
| 7     | **模型仍是一块面板而不是 R10 要求的一颗 chip**。收成「标签即当前模型 ▾」要改 studio-shared 的 `BaseModelPickerPanel`，属跨线项（同 §8 末段那条线）。十族已统一落进编排台槽，只差形态      | owner / 另立片 |
| 8     | **videoReference 的素材架整栏不渲染**。§6 那一行把「上传/替换」同时列进素材架与动作坞 —— 那是同一颗按钮出现两次，且违反 R10「全屏只有一个实心元素」。实现按「它就是这一屏的主事」归动作坞 | owner 复核该行 |

### 14.1 实现状态（2026-08-04）
...
| --------------- | ------------------------------- | ------------------------- | -------------------------------------------------------------------------- |
| 参考图 + LoRA   | E 原型画在**编排台**            | 归**素材架**              | §6 那张表自己写的是「素材架: 素材库/Studio + 参考图·LoRA」；槽定义也指向它 |
...
2026-08-04 · **方向 E 已全量实装**（S1–S8，十族）。§14 加了实现状态与三处
「按契约自行判定、与原型不一致」的登记。
