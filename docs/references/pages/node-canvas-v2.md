...
10. **双击 = 展开 / 快速看**：文本、视频卡双击 = 展开画中框；图片卡双击 = 快速看图（原比例居中的大图，只有版本、下载、关闭，没有参数）；快速看片走空格 / ⋯ 菜单。Esc 或点空白关闭。**提示词栏、参考轨、工具条内的双击不冒泡到卡片**（四类卡通用，2026-09-10 owner 定）。
...
- 参考轨 = 提示词栏首行，三组（图 · 视频 · 语音）以细分隔线分开，每项 48px 编号缩略（图角标 首 / 尾），每组末尾一个虚线加号 → 画布上的卡 › / 上传 / 从素材库（素材库按组过滤）。上限按模型能力表（满了加号灰、不藏）；组内序号就是 @ 的号：`@图1` / `@视频1` / `@语音1`，删一项后面顺位。展开态的画中框在播放器与说明之间摆同一条轨。
