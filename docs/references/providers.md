...
| fish_audio        | 音频 TTS（**s2.1-pro-free** 免费档，2026-08-31 到期）                        | **无 getSystemApiKey 平台 key 映射**（BYOK-only 现状）。稳定 key 仍是 `fish-audio-s2-pro`，只换 `externalModelId`。⚠ 免费期到期若不再延，这个 model string 会失效，而它是唯一 `available: true` 的语音模型——回退到收费档 `s2.1-pro` 的做法见 `models/audio.ts` 上的注释                                                                                                             |
