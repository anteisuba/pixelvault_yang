...
助手图片与工作台提示词使用同一份宿主参考图列表。上传、选素材、拖入或结果投递均写入该列表；两边同步编号、缩略图与移除，发送后保留。`@` 只列启用图片，超容量图片保留并标为不可用；音频与视频附件继续走原附件通道。

图片助手每轮读取当前参考图的编号与 URL，以当前顺序取代历史编号。`set_prompt` 将 `Image1`、`图1`、`参考图1` 等引用统一为 `@Image1`，复用提示词输入框的行内缩略图；引用不存在或仍待导入的图片时拒绝写入。画风指导要求区分人物、服装、姿势与主要画风来源，缺少视觉证据时先检查用户明确提到的画风图，并按可观察特征描述。该指导是自然语言约束，不代表模型支持画风硬锁或逐图权重。

图片域的 `analyze_references` 只提取已挂载源图的视觉事实，不强制分配创作用途。可用零起点 `imageIndices` 指定图片（图3为 `[2]`），省略时检查全部；仅把指定范围内尚未分析的图片送入视觉调用，描述身份与服装、姿势、比例/轮廓/明暗/材质/色彩/光照、场景和不确定项。补分析的图片保留当前列表编号，模型返回编号后由程序绑定原始 URL；不评价源图是否符合待生成目标。视觉记录随助手历史保存，换序按 URL 重新绑定、换图只补新图。纯视觉结果的 `brief` 为 `null`，卡片直接展开事实；已有分工卡仍显示其分工。JSON、字段和图片编号校验失败分别记录阶段、路径及编号，不记录模型原文；这些错误不作为要求重新上传的依据。源图 HTTP 读取失败仍明确指出当前图号，禁止推断未读内容。

带参考图的 `set_prompt` 必须先取得当前顺序的完整视觉事实，再按最新创作要求构建来源分工、保留项、排除项和创作要求。分工输出使用编号，由程序映射 URL；分工校验失败保留视觉事实、不修改提示词，并显示独立错误。分工有未解歧义时先询问。写入前检查完整最终提示词（含追加后的原文）的语义冲突，返回具体遗漏、错配或改变创作要求的补充；两次检查失败后停止继续付费检查；同一轮通过检查并写入一次后，拒绝后续重复改写，新一轮用户指令可继续修改。程序同时检查图片引用是否存在。`critique_result` 拒绝把已挂载的源图当成失败成图评价；实际成图评价同时接收结果图与当前参考图进行对照。以上约束用于减少提示词偏离，不保证生成模型逐次遵从。
...
- 提案卡三档纪律见 [`node-canvas-v2.md`](node-canvas-v2.md) §8.3，本文不重复。
...
| 2   | **撤销回滚画布**              | 薄卡二选与系统行文案与图片域**完全一致**                                                                                                                                                                                         | 助手的一轮 = **一个 undo 条目**（事务式 push，不是 N 条），按 `inverse` **逆序**执行；纯视图状态不进 undo 栈。画布侧已落地：栈只有一份、住在 `use-node-graph-v4.ts`，不进栈的例外只有四类（见 `node-canvas-v2.md` §6.3） |
...
| 5   | **边进快照**                  | 快照按镜号排序，边**内联在目标节点下**（`slot ← 源`），跨镜边单列 `# 接续`；**分层送**（owner 2026-09-06）：当前镜 + 相邻两镜 + 选中 + 最近改动 = 完整结构，其余镜头只送一行标题，**不设节点数硬上限**（`node-canvas-v2.md` §4） | 已落地：快照带边、分层、文本槽按角色分行、轮播槽只报当前版；`NODE_STUDIO_ASSISTANT_LIMITS.maxNodes` 现在是**标题档的行数上限**，不再截断完整档                                                                           |
