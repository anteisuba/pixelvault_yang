...
| Workflow                | 触发                                                                                                                               | 内容                                                                                                                                                                                                                                                                                              |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`                | push main/feat/\* · PR→main                                                                                                        | **四个 job**：lint（prisma generate → `tsc --noEmit` → eslint）· test（unit，`vitest run --reporter=verbose`）· audit（`npm audit` 容忍失败 + Prisma migration drift check，带 Postgres 16 service）· build（生产构建，needs lint+test，限时 10min）；Node 22 + npm cache，前三个 job 各限时 5min |
...
- Date: 2026-07-10 · Method: 逐 workflow 读取触发与步骤；`models:check-docs` 基线已补（见上）且脚本存在 0 模型解析退化（经 git ls-tree + 实测快照核验）。
