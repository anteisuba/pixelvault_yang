...
日期：2026-06-11
状态：**草案，待所有者确认**。确认前不据此大规模动工；与 `reviews/2026-06-11-ui-audit-pass1-code.md` 配套阅读。
...
1. **双面模式**（创作面深色 + 陈列面米白）vs 全站统一深色——本草案推荐双面，理由：贴合现有架构、贴合"个人美术馆"隐喻、MeiGen/Recraft 验证了浅色陈列的效果。
2. **无彩到底**：主 CTA 用黑/白反相按钮，不引入品牌彩色——确认接受？
3. **落地顺序**是否按上表执行（样板间 = Studio）。
