...
> 现行体系：**v1 暗房工作台**（2026-07-02 评审拍板，`docs/archive/reviews/2026-07-02-lora-domain-ui-review.md`，含 D1–D9/B1–B11）。本次是**功能性重设计**（检索维度 + 双源收敛 + 组合体验），**不是房间改版**——紫矿「釉料房」是 brand-dna 房间层草案、主轴重议中，本文不引入其任何隐喻/颜料/手写字体，全部沿用 v1 现状约定（无彩灰阶 · 黑纱徽标 · 唯一反相 CTA · 琥珀仅警示 · 暗房工作台去盒化语言）。
...
| **S3 统一卡片+抽屉**                                | `LoraLibraryCard` + `LoraLibraryInspector`（HF file select 迁入、收藏语义统一）                                               | 双源卡片同基底仅源角标异；HF 多文件 repo 在抽屉选文件后收藏成功、未选时主按钮引导；civitai 抽屉全区块零回归（授权徽标/试用词/带词去生成）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **S4 组合体验**                                     | 兼容圆点 + 警示行（含建议底模动作）+ 常同挂行 + 底模 Select 两层分组（§4.4）                                                  | 挂 Illustrious LoRA + 切 Flux 底模 → 圆点亮 + 警示行现「切到 WAI Illustrious · 忠实」→ 点击即切且警示消失；双家族互斥挂载 → 提示卸载不给假建议；底模下拉见 云端/Runner 两组、runner 组内 SDXL/DiT 再分；有共挂数据的 LoRA 见 Top3 行、点 +挂载 入栈；无数据不渲染                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **S5 触发词 chips 化**                              | TriggerChipRow + prefill 迁移 + 编译顺序                                                                                      | 挂 2 个带触发词 LoRA → 2 chips、正文空；出图请求 prompt = 触发词+tray+正文序；停用一枚后请求不含它；一键同款不碰 chips 行                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **S6 inline 补全**                                  | PromptTagAutocomplete                                                                                                         | 正文键入 `long` → 浮层含 Long Hair；↑↓Enter 插入 `long_hair, `；中文 IME 拼音阶段不弹；Esc 后同词段不再弹；负向框补全负向 tags                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- 规矩：`docs/brand-dna.md`（过渡期规则）· `docs/forbidden.md`（UI 节）· `docs/references/frontend.md`
