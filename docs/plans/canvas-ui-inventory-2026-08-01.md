...
### 21.1b owner 拍板：统一与各族自定的分界（2026-08-03）

owner 原话：「单独设计的是**保留什么，布局。以及按钮点击后的样式**。」

| 统一（所有族一套）            | 各族单独设计                                              |
| ----------------------------- | --------------------------------------------------------- |
| 骨架 · 材质 · 间距 · 状态语言 | ① 保留什么（哪些内容块留下）· ② 布局 · ③ 按钮点击后的样式 |

⚠ ③ 有两种读法，设计会话阶段 3 要拿真界面确认一次：
**A**「点完面板变成什么样」（哪些块出现/消失、怎么落位）—— 与②连续，各族自定天经地义，
且现状最烂的两处正属于它（AI 表单折叠 700px 零过渡、音色面板整树替换）；
**B**「按钮自己被按下时长什么样」—— 若按 B 走成各族自定，会与「材质统一」和
§17.3 的状态强度梯度直接冲突（同一控件两副面孔）。
**交接书里按 A 记，B 归「材质/状态语言统一」那一栏。**
