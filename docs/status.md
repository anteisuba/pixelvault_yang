...
- 镜头失败反馈补漏：生成错误（原文/错误码/翻译键）随项目保存，卡片、展开态和移动端持续显示；旧失败节点显示通用错误，重试保留已有视频，开始新任务或成功替换素材后清除旧错误。补充 75 项定向测试、类型和目标 lint 通过；全量 718 文件、8287 项通过（1 项跳过）。按此前授权准备发布，不发起真实付费生成。
- 画布节点本地修复：镜头展开前后共用提示词，素材轨改为横向缩略图与统一添加菜单；画布上传显示逐文件加载态，文字节点支持双击原位编辑。生成失败清任务 ID、状态回填按最新图合并，镜头取消接入服务端并保留已完成产物；取消终态停止轮询，旧任务回包不覆盖新任务。96 项画布定向测试与类型检查、目标 lint 通过；桌面浏览器已验证提示词往返、素材菜单及文字编辑，上传加载态用并发延迟测试验证，未发起真实付费生成。线上 eva 镜头对应 Worker 已完成失败回传，无在跑任务需要取消。本轮独立提交在隔离工作树验证：718 个文件、8190 项测试通过（1 项跳过），全量类型检查通过，lint 0 错误、14 条既有警告；原位编辑已关闭聚焦时自动滚动。仅提交画布修复，生产构建与移动端端到端发布检查待完成，未 push/发布。
...
- 本轮规则审计：对照官方 GPT-6 Astra/Codex 指导，精简 AGENTS、WORKFLOW、两客户端技能与配套场景；改为按授权自主推进、按需加载和按影响面验证。保留 UI 方向确认、数据与发布边界，移除有冲突的生产迁移操作示例。仅文档修改，尚未 commit/push。 验证：64 个技能入口 YAML/名称/描述有效，23 对原本相同的技能副本无分叉；本轮 Markdown 引用、格式和 diff 检查通过。官方 quick_validate 缺 PyYAML，使用仓库现有 YAML 解析器完成结构检查，保留客户端原有扩展字段；未运行应用测试或模型性能基准。
