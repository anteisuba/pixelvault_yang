...
> 🔀 **上面这两条正在被复议，决议在阶段 6-B —— 别在决议之前照着它们新增投影，也别自作主张先删。**
>
> 度量表 [`canvas-libtv-form-metrics-2026-08-09.md`](../../plans/canvas-libtv-form-metrics-2026-08-09.md) §5 #3：参考站是**零投影**（全站只有选中动作条一层 `0 4px 10px`），层次全押在**表面等级 + 1px 发丝边**上 —— 与本文这两条正好相反，是**策略级反转**不是数值微调。
>
> 两边都有硬理由：本文这两条是从**实测反例**来的（同屏 7 套白玻璃差值 1.00–1.03），参考站那套是从**实测正例**来的。所以它不是「谁抄谁」的问题，要在 6-B 里连同 §5 #1（节点卡直角无边无影）一起定 —— **那两条是同一道题**：卡的边一去掉，边界就只剩 `--canvas-card-bg #FFFFFF` 对 `--canvas-bg #F1F1F1` 的 **1.13:1** 撑着。
>
> ✅ 6-B 的前置**比原先估的近**：本节 S0/S1/S2 的色值早已存在且过了 `contrast-check`（1.13 / 1.16 / 1.31，门槛 1.05，2026-08-04 确定性脚本）。真正没算的只有**画布底那一层**。
>
> 阶段 6-A（2026-08-10）已经把**纯尺度**的四条采纳掉了（字号 / 控件高 / 圆角 / 连线），刻意没碰分层，就是为了不让人在这道题定之前先动手。
