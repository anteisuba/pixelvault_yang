...
- **minimap 仍是深色**（`rgb(16,24,32)`，节点标记深色叠深色）—— 白底画布上一个黑洞。S2b 只改了 `--canvas-minimap-left` 让位，没重映射配色。
- **打开画布时边一条都不画** —— ✅ **2026-07-26 已复验，是真 bug；但原描述「等视口变化才出现」是错的。**
  ...
  **定性**：首次挂载没算 handle bounds。**修法**：`nodesInitialized` 翻 true 后对全部节点 `updateNodeInternals()` 一次。⚠ 是不是 S1/S3.5 引入的**尚未归因**，修的时候顺手看一眼。
