...
- **Asset Library / Dataset Workspace**：Future Direction，**不在 Phase 1-3**（待單獨訪談）
...
| **F20** | **Future Direction（記錄）** | **Asset Library / Dataset Workspace**：用戶上傳本地圖片/文件夾 → 組織為可搜索可篩選可批量管理的資產集合 → 訓練時選素材集 → 生成結果加入素材庫。**不在 Phase 1-3**，待單獨訪談 | Q17 補充            |
...
#### **F20 · Asset Library / Dataset Workspace**

- **能力範圍**
  - 用戶可上傳本地圖片或整個文件夾到網站
  - 素材組織為可搜索、可篩選、可批量管理的資產集合
  - 訓練（LoRA）時可直接選擇某個素材集作為訓練來源
  - 生成結果可一鍵加入素材庫，作為後續訓練或 reference 使用
- **狀態**：Future Direction，**不在 Phase 1-3**
- **觸發條件**：
  - Phase 1-3 image-transform 主線穩定後
  - 用戶對 LoRA 訓練素材管理提出明確需求
  - 或運營發現「用戶反覆上傳同樣的 reference 圖」成為 top pain point
- **潛在耦合**
  - 與現有 LoRA 訓練能力（`WP-LoRA 訓練`）強耦合
  - 與 Gallery / Profile 的「加入收藏 / 訓練集」操作需新設計
  - 與 F12 Subject Card 體系可能合流（Subject Card 本質是 Asset Library 中的一類資產）
- **不做決策**：待單獨訪談
