...
5. **Studio 工具栏 IA** ✓（2026-06-11）——目标工具栏 6 chips：`[LLM助手·名称待定] ｜ 图像(含提取风格) ｜ 变换 ｜ 卡片(含风格) ｜ LoRA ｜ 1:1`。分项：①「风格」预设并入卡片→画风页签（成为内置画风卡，风格概念从 5 处收敛到 1 处）；②「增强」重定位为纯文本 LLM 助手——砍风格类预设、保留动作类（扩写/转标签/翻译/看图描述）+ 空态起手势 + 产出三动作（填入/追加/复制），多 LLM 路由是其特色，命名待定（候选见 reviews 讨论）；③**以图定风格 = 提取风格→可存画风卡**（风格资产化）：图像面板加"提取风格"动作（复用反推 artStyle 维度），就地填 prompt + 可另存画风卡；卡片管理的画风卡新增"从图片创建"入口；通用五维反推降级为"更多拆解"高级选项。变换+反推合并为「图像变体」记为方案 B，验证手感后再议。LLM 助手定名**「助手」**（zh 助手 / en Assistant / ja アシスタント，2026-06-11 确认）。

**工具面板统一契约**（决议 5 附则——所有工具面板向此收敛，新面板必须遵守）：

1. **披露分级**：轻面板（快速配置/选择，单屏内容）→ `StudioToolSurface` 锚定 chip 上方（手机自动变底部抽屉）；重面板（多轮对话/大列表/多步流程）→ 居中 Dialog（手机走 ResponsiveDialog 抽屉）。判据：内容是否需要持续停留与滚动。
2. **统一 chrome**：两类面板共用 `tool-surface` 导出的暗面外观——popover 型用 `StudioToolPopoverContent`，Dialog 型用 `studioDialogBaseClass / studioDialogHeaderClass / studioDialogBodyClass`；头部规范 = 图标 + 标题（+ 状态 badge），border-b 分隔。
3. **按钮层级**：面板内主动作 = muted 家族 pill（`bg-muted/65 → hover:bg-muted`）；选中态 = `bg-primary/10 text-primary ring-primary/30`；黑/白反相丸**只属于「生成」**；destructive 红仅删除。
4. **选项 pill 语法**：所有"从 N 个里选一个"（预设/比例/风格/维度）统一为 `rounded-full px-2.5 py-1 text-2xs` pills，同选中态。
5. **空态起手势**：每个面板空态 = 一句说明 + 可点的起手动作（示例气泡/「去管理」入口），不留死空白。
6. **产出三动作**：AI 产出（助手/反推/提取风格）统一「填入 / 追加 / 复制」三动作语法，复制成功原地变勾 2 秒。
7. **chip 状态**：有活动状态的 chip 右上角统一 `bg-primary` 数字徽标；当前模型不支持的能力不渲染 chip（不留死按钮）。
