// 量每张画板的实际高度 → 写 canvas-live.json（页 · 位置 · 尺寸）与 args.txt（seed 参数）。
...
const COLS = { 'page-1': 1, 'page-2': 2, 'page-3': 3, 'page-4': 1, 'page-5': 3 }
...
for (const pg of PAGES) {
  for (const board of pg.boards) {
    ...
    heights[board.file] = await page.evaluate(() => document.documentElement.scrollHeight)
  }
}
await browser.close()
...
for (const pg of PAGES) {
  ...
  for (let i = 0; i < pg.boards.length; i += cols) {
    ...
    row.forEach((b, j) =>
      artboards.push({ file: b.file, x: j * (WIDTH + GAP_X), y, w: WIDTH, h: heights[b.file] + 20, title: b.title, page: pg.id }),
    )
    y += rowH + GAP_Y
  }
}
...
writeFileSync(join(HERE, 'canvas-live.json'), `${JSON.stringify(canvas, null, 2)}\n`)
writeFileSync(join(HERE, 'args.txt'), artboards.map((a) => `--artboard\ndigest/${a.file}\n`).join(''))
