    ...
    h('业务地图与依赖', '上一层不定，下一层展不开；同层可并行'),
    {
      ...
      layers: [
        ...
        },
        {
          ...
          why: '都只依赖底座',
          items: [
            { name: '助手', status: '部分', note: '公共层与四张脸已落；D12 图片助手 09-24 施工完、两条验收（只换衣服 · 三渲二 → 赛璐璐）实跑通过；「拆分与反推」09-24 施工完、五态实跑通过（待上线）' },
            ...
            { name: '视频', status: '部分', note: '模型接入已落；视频助手 + 左栏 09-24 施工完，BytePlus 2.5 与 Wan 3.0 实跑出片（待上线）；分镜 / 白模动作等 D4' },
  ...
  [
    ...
    table(
      ...
      [
        ...
        ['—', '拆分与反推', '把用户的话拆成 NAI 标签；参考图反推成自然语言或标签，尽量复刻（09-24 owner 提出）；④ 画板在第 6 页，施工完实跑 X1–X8 通过（b8300847）；刷新后不再误问覆盖、灰字进历史（90b84b54 · eff047d0）', '待上线'],
        ...
        ['—', '步数用完说清剩余再停', '最后一步强制收尾 + 「继续」', '待上线'],
        ['—', '3D 图被说成 2D', '09-24 改成逐条核对、按多数判；三渲二参考图实测判成风格化 3D', '待上线'],
  ...
  [
    ...
    table(
      ...
      [
        ...
        ['—', '视频助手 + 左栏', '④ 画板在第 6 页；94e058b5 施工，实跑 BytePlus 2.5 · Wan 3.0（fal）出片；遗留：从头写提示词偶发连写三遍', '待上线'],
...
const PROGRESS = {
  ...
  sub: `只列没做完的；已完成的压在最后一段。# 沿用原编号，方便对 commit。${BASE}。当前焦点：D12 图片助手、「拆分与反推」、视频助手 + 左栏都已施工完待上线；下一件建议卡片 35，它挡着画布。`,
  blocks: [
    { t: 'legend' },
    h('层 0 · 底座'),
    ptable([
    ...
    ptable([
      ...
      P('—', '拆分与反推：把用户的话拆成 NAI 标签；参考图反推成自然语言或标签，尽量复刻（09-24 owner 提出）；9f6f8a22 · b8300847 · 90b84b54 · eff047d0', '—', '待上线'),
    ...
    h('层 1 · 视频'),
    ptable([
      P('—', '视频助手 + 左栏施工完（按模型写原生格式 · 素材轨 · 发送方式按挂载推 · 失败原因说具体 · 结果卡说视频）；实跑 BytePlus 2.5 · Wan 3.0 出片；94e058b5 … 14f65dca，未推生产', '—', '待上线'),
