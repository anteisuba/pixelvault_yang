...
规格版 HTML（base64 已替换为下列路径）曾在 `docs/plans/prototypes/homepage-slide-v2-SPEC.html`，
已随 `docs/plans/` 于 2026-09-01 全清删除，需要时从 git 历史取。

⚠ **2026-08-28 P1 已把 `asset-NN.*` 全部改成语义名**（映射表见文末）。那份 SPEC HTML 里写的仍是旧的
`/homepage/v4/asset-NN.ext`——**它是历史规格快照，不再更新**；引用资产一律以本文件的语义名为准，
或直接读 `src/constants/homepage-v4.ts`。
