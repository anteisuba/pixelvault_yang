...
- Date: 2026-07-23 · Method: 核验应用 Vitest、Execution Worker 独立 Vitest、typecheck、changed-file ESLint 与 CI 闸门。全量应用测试在并发负载下仅有两个未改 UI 文件超时，两个文件隔离重复运行均 14/14 通过；不能把这种隔离结果表述为全量全绿。
