...
/**
 * 视频模型的展示顺序 —— 也是**默认渠道**：用户没有自带 key 时，
 * `pickDefaultVideoModel` 取同一型号里排最前的那一条（`pickBest` 的
 * `candidates[0]` 兜底），选择器第三层的渠道排序也吃这张表。
 *
 * ⚠ Seedance 一律**火山方舟在前、fal 在后**（owner 2026-09-06）：同一个模型
 * 走 fal 转售约 2.1× 成本，没道理让没配 key 的用户默认落在贵的那条上。
 * BYOK 不受影响 —— 用户自己配了 key 的渠道永远压过这张表。
 */
const VIDEO_MODEL_PRIORITY: Partial<Record<AI_MODELS, number>> = {
  [AI_MODELS.SEEDANCE_20_FAST_VOLCENGINE]: 1,
  [AI_MODELS.SEEDANCE_20_FAST]: 1.5,
  [AI_MODELS.SEEDANCE_20_FAST_BYTEPLUS]: 1.75,
  [AI_MODELS.SEEDANCE_20_VOLCENGINE]: 2,
  [AI_MODELS.SEEDANCE_20]: 2.5,
  ...
  [AI_MODELS.SEEDANCE_20_REFERENCE_VOLCENGINE]: 7,
