...
const PROGRESS = {
  ...
  sub: `只列没做完的；已完成的压在最后一段。# 沿用原编号，方便对 commit。${BASE}。当前焦点：D12 图片助手已施工完待上线，「拆分与反推」也已施工完待上线；建议卡片 35 同时开，因为它挡着画布。`,
  blocks: [
    { t: 'legend' },
    h('层 0 · 底座'),
    ptable([
...
const VIDEO_ASSISTANT = {
  ...
  heading: '视频助手 + 左栏：全状态',
  sub: 'owner 09-24 ① 八问：时间轴「镜头N（a-b秒）」两种都写（09-24 追加：**按模型写它自己的格式**，见④）· 写作分两档（简单需求 / 剧本）· UI 范围 = 左栏参数区 + 助手视频脸 · 去掉三个模式、按挂了什么判断 · 素材按图片1 / 视频1 / 音频1 编号 · 分镜是纯文本逐行 · 规格行只写一行短规格。anima-tagger 的 Anima 格式规则放 LoRA 侧，里面的 Danbooru 校验规则留给 NAI 标签检查。助手面板沿用「A 对话流：全状态」，新画面只有**左栏**和**规格浮层**。',
  blocks: [
    h('① 左栏：现在 → 改后'),
