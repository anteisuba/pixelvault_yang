...
| 片               | 内容                                                                                                                         | 依赖                  |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| S1 五卡重组      | DEPARTMENTS 表改五制作角色；角色槽挂音色徽标（含 ＋配音 autospawn voice→character）；旁白卡（voice 直连）；keyframe 归镜头卡 | 无（纯重组+复用引擎） |
| S2 @token 原子化 | 最小 mention-input 替换 prompt textarea；删漂移机制                                                                          | 独立，可与 S1 并行    |
| S3 运镜 L1       | 运镜语法▾ chips（cinematic-grammar 词表）                                                                                    | 无                    |
| S4 运镜 L2       | 镜头卡结构化 camera 下拉                                                                                                     | S1                    |
| 后续             | 超限⚠（先抽共享 payload 装配）· 拖拽排序（先定排序建模）· 上传/粘贴入口                                                      | 见 v4 §14.3           |
